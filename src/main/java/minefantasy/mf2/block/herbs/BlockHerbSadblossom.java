...
public class BlockHerbSadblossom extends BlockHerbsMF {
    private int meta, maxMeta = 1;
    private String name = "sadblossom";
    @SideOnly(Side.CLIENT)
    private IIcon[] icons;

    public BlockHerbSadblossom() {
    ...
    public boolean interract (World world, int x, int y, int z, EntityPlayer user) {
        ...
        if (held != null && held.getItem() instanceof ItemShearsMF && meta == 1) {
            dropItem(world, x, y, z, ComponentListMF.sadblossom_item, 3, true, true);
        ...
        if (meta == 1) {
            dropItem(world, x, y, z, ComponentListMF.sadblossom_item, 2, true, false);
            --meta;
            world.setBlockMetadataWithNotify(x, y, z, meta, 2);
            return true;
        }
