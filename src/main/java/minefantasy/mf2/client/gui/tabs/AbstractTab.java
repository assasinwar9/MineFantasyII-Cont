...
public abstract class AbstractTab extends GuiButton {
	ResourceLocation texture = new ResourceLocation("minefantasy2:textures/gui/tabs.png");
	ItemStack renderStack;
	RenderItem itemRenderer = new RenderItem();

	public AbstractTab(int id, int posX, int posY, ItemStack renderStack) {
		super(id, posX, posY, 32, 28, "");
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY) {
		if (this.visible) {
			...
			int yTexPos = this.enabled ? 28 : 0;
			// int ySize = this.enabled ? 25 : 32;
			// int xOffset = this.id == 2 ? 0 : 1;
			//int yPos = this.yPosition + (this.enabled ? 3 : 0);

			mc.renderEngine.bindTexture(this.texture);
			this.drawTexturedModalRect(this.xPosition, this.yPosition, 0, yTexPos, 32, 28);
